...
<dl>
    ...
    <dd>
       ...
       name="wr_1"
       id="wr_1"
       value="<?=get_text($write['wr_1'])?>"
       class="frm_input full"
       size="50"
       maxlength="255">
    </dd>
</dl>
...
<dl>
	<dt>기록일</dt>
	<dd><input type="text" name="wr_date" id="wr_date" value="<?php echo $wr_date_value; ?>" class="frm_input full" maxlength="10" placeholder="YYYY-MM-DD"></dd>
</dl>
<dl>
	<dt>일일 목표</dt>
	<dd>
...
<?php if (!$is_plain_board) { ?>
	function updateWrDoneField() {
		var doneIds = [];
		$('.goal-item.done').each(function(){
			doneIds.push($(this).data('wr-id'));
		});
...
$(function() {
    ...
    function bearSyncIfPossible() {
        try {
            if (typeof window !== 'undefined' && typeof window.bearEditorSync_wr_content === 'function') {
                window.bearEditorSync_wr_content(); // 편집영역 -> hidden textarea 동기화
                return true;
            }
        } catch (e) {}
        return false;
    ...
    function getEditorContent() {
        // 1) BearEditor 우선
        // - sync 함수가 있으면 textarea(#wr_content)에 최신값이 들어감
        if (bearSyncIfPossible() && $('#wr_content').length) {
            return $('#wr_content').val();
        }

        // - sync가 없으   contenteditable 영역을 직접 읽음
        if (document.getElementById('wr_content_editor')) {
            return document.getElementById('wr_content_editor').innerHTML;
        }

        // 2) SmartEditor2 (구버전 호환)
        if (typeof oEditors !== 'undefined' && oEditors.getById && oEditors.getById["wr_content"]) {
            return oEditors.getById["wr_content"].getIR();
        }

        // 3) CKEditor 호환(구버전 호환)
        if (typeof ed_wr_content !== 'undefined' && ed_wr_content && typeof ed_wr_content.getData === 'function') {
            return ed_wr_content.getData();
        }
    ...
    function setEditorContent(content) {
        ...
        // 1) BearEditor 우선: contenteditable + textarea 둘 다 갱신
        if (document.getElementById('wr_content_editor')) {
            document.getElementById('wr_content_editor').innerHTML = content;
        }
        if ($('#wr_content').length) {
            $('#wr_content').val(content);
        }

        // editor.js 쪽에서 별도 상태/카운트 업데이트가 필요할 수 있어 이벤트 한번 쏴줌
        try {
            if (document.getElementById('wr_content_editor')) {
        ...
        // 2) SmartEditor2 / CKEditor 호환 (혹시 같이 쓰는 경우)
        try {
            if (typeof oEditors !== 'undefined' && oEditors.getById && oEditors.getById["wr_content"]) {
    ...
    $.post(g5_bbs_url + '/write_temp_save.php', {
        ...
        subtitle: $('#wr_1').val(),
        content: (function(){
            // BearEditor 우선: hidden textarea로 동기화 후 값 읽기
            try {
                if (typeof window.bearEditorSync_wr_content === 'function') {
            ...
            // fallback: contenteditable 직접 읽기
        ...
        // write_temp_save.php가 'OK'를 반환하므로 그 케이스도 처리
        if (typeof res === 'string' && $.trim(res) === 'OK') {
            alert('임시 저장 완료');
            // 카운트 업데이트 함수가 있으면 호출
            if (typeof updateTempCount === 'function') updateTempCount();
            return;
        }
        ...
        if (res && res.success) {
            ...
            alert('임시 저장 실패: ' + res.message);
        } else {
            // 문자열 OK가 아닌데 여기로 오면 서버가 HTML을 반환했을 수도 있음(로그인 페이지 등)
            alert('임시 저장 실패(응답 확인 필요)');
            console.log(res);
        }

    }).fail(function(xhr) {
        alert('임시 저장 실패! 서버 확인 필요');
    ...
    $('#btn-temp-list').on('click', function() {
        $.getJSON(g5_bbs_url + '/write_temp_list.php', { bo_table: '<?php echo $bo_table ?>' }, function(list) {
            let html = `
                <button type="button" id="btn-temp-close" style="float:right; width:30px; background:#000; font-size: 14px; color:#fff; border:none; cursor:pointer;">✖</button>
                <h3 style="color:#fff; margin-bottom:15px;">임시 보관함</h3>
                <label style="color:#eee;"><input type="checkbox" id="temp-select-all"> 전체선택</label><br><br>
            `;
            ...
            (list || []).forEach(v => {
                ...
                pureText = pureText.replace(/\xa0/g, ' ').trim();

                let countWithSpace = pureText.length;
                let countWithoutSpace = pureText.replace(/\s/g, '').length;

                html += `
                    <div class="temp-item" data-id="${v.id}" style="padding:10px 5px; border-bottom:1px solid #444; cursor:pointer; position:relative; color:#fff;">
                        <strong style="display:block; font-size:14px; padding-right:30px;">${v.subject || '제목 없음'}</strong>
                        ${v.subtitle ? `<span style="display:block; font-size:13px; font-weight: bold; color:#ddd; margin-top:4px;">${v.subtitle}</span>` : ''}
                        <small style="color:#888; font-size:11px; display:block; margin-top:2px;">${date}</small>
                        <span style="display:inline-block; margin-top:5px; padding:2px 0px; background:none; font-size:11px; color:#999;">
                            약 ${countWithoutSpace}자 / ${countWithSpace}자 (공미포/공포)
                        </span>
                        <input type="checkbox" class="temp-check" data-id="${v.id}" style="position:absolute; top:12px; right:5px;">
                    </div>
                `;
            });
            ...
            $('#temp-select-all').on('change', function() {
                $('.temp-check').prop('checked', $(this).prop('checked'));
            });
        }).fail(function(xhr){
            alert('임시 보관함 불러오기 실패');
            console.log('write_temp_list fail', xhr.status, xhr.responseText);
        });
    ...
    $(document).on('click', '.temp-item', function(e) {
        ...
        $.getJSON(g5_bbs_url + '/write_temp_load.php', {
            bo_table: '<?php echo $bo_table ?>',
            id: id,
            t: new Date().getTime()
        }, function(v) {
            ...
            alert('임시글 불러오기 실패');
            console.log('write_temp_load fail', xhr.status, xhr.responseText);
        });
    });
    ...
    $(document).on('click', '#btn-temp-delete', function() {
        ...
        // bo_table 같이 보내는 편이 안전 (서버가 bo_table로 스코프를 나누는 경우 대비)
        let pending = ids.length;
        ids.forEach(id => {
            $.post(g5_bbs_url + '/write_temp_delete.php', { bo_table: '<?php echo $bo_table ?>', id:id })
             .always(function(){
                pending--;
                if(pending === 0){
                    alert('삭제 완료');
    ...
    $(document).on('click', '#btn-temp-close', function() {
        $('#temp-layer').hide();
    });
